...
public class DisplayStatementImpl extends StatementImpl implements DisplayStatement {
	...
	public Operand addOperand(final DisplayOperandContext ctx) {
		...
		if (result == null) {
			...
			// operand
			final Call operandCall;

			if (ctx.identifier() != null) {
				operandCall = createCall(ctx.identifier());
			} else if (ctx.literal() != null) {
				operandCall = createCall(ctx.literal());
			} else if (ctx.otherKeyword() != null) {
				operandCall = createCall(ctx.otherKeyword());
			} else {
				operandCall = null;
			}
